...
export function registerSessionIpc(
  ...
  /** 归档项目 / 对话专用硬删除：直接移除数据库记录，不留下 deleted 软删除行。 */
  ipcMain.handle('local-db:sessions:permanent-delete-archived', async (_e, ids: unknown) => {
    if (!Array.isArray(ids)) throwIpcError('INVALID_PARAMS', 'sessionIds must be an array');
    const sessionIds = Array.from(new Set(ids.map((id) => requireString(id, 'sessionId'))));
    if (sessionIds.length === 0) return { deleted: 0 };
    const dbClient = getDbClient();
    const placeholders = sessionIds.map(() => '?').join(', ');
    const rows = await dbClient.query<{ id: string; status: string; workspace_kind: string }>(
      `SELECT id, status, workspace_kind FROM sessions WHERE id IN (${placeholders})`,
      sessionIds,
    );
    if (
      ...
      rows.some(
        (row) =>
          row.status !== 'archived' ||
          (row.workspace_kind !== 'project' && row.workspace_kind !== 'dialogue'),
      )
    ) {
      throwIpcError(
        'PRECONDITION_FAILED',
        'only archived project or dialogue sessions can be permanently deleted',
      );
    }
    // 共享库中的永久删除必须清理完整关系链，不能只删 sessions。
    const tableRows = await dbClient.query<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type = 'table' AND name NOT LIKE 'sqlite_%'",
    );
    for (const { name: table } of tableRows) {
      if (['sessions', 'messages', 'media_blobs', 'media_refs'].includes(table)) continue;
      const tableInfo = await dbClient.query<{ name: string }>(
        `PRAGMA table_info("${table.replaceAll('"', '""')}")`,
      );
      const hasSessionId = tableInfo.some((column) => column.name === 'session_id');
      const hasOriginSessionId = tableInfo.some((column) => column.name === 'origin_session_id');
      if (!hasSessionId && !hasOriginSessionId) continue;
      const predicates: string[] = [];
      if (hasSessionId) predicates.push(`session_id IN (${placeholders})`);
      if (hasOriginSessionId) predicates.push(`origin_session_id IN (${placeholders})`);
      const params = Array.from({ length: predicates.length }, () => sessionIds).flat();
      await dbClient.exec(
        `DELETE FROM "${table.replaceAll('"', '""')}" WHERE ${predicates.join(' OR ')}`,
        params,
      );
    }
    try {
      await dbClient.exec(
        `DELETE FROM media_refs WHERE origin_session_id IN (${placeholders})`,
        sessionIds,
      );
      await dbClient.exec(
        `DELETE FROM media_refs WHERE ref_kind = 'message' AND ref_id IN (SELECT client_id FROM messages WHERE session_id IN (${placeholders}))`,
        sessionIds,
      );
    } catch {
      // 媒体引用表是可选扩展。
    }
    await dbClient.exec(`DELETE FROM messages WHERE session_id IN (${placeholders})`, sessionIds);
    await dbClient.exec(`DELETE FROM sessions WHERE id IN (${placeholders})`, sessionIds);
    try {
      await dbClient.exec(
        'DELETE FROM media_blobs WHERE NOT EXISTS (SELECT 1 FROM media_refs WHERE media_refs.hash = media_blobs.hash)',
      );
    } catch {
      // 媒体表是可选扩展，缺失时不影响会话删除结果。
    }
